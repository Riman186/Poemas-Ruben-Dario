...
let recognition;
let currentLine = 0; // Usaremos este como el índice de la línea actual
let poemLines = [];
...
recordBtn.addEventListener("click", async () => {
  if (!mediaRecorder || mediaRecorder.state === "inactive") {
    try {
      ...
      // Conectamos la fuente al analizador
      source.connect(analyser);
      // Creamos un "silenciador" y lo conectamos al destino final para que el audio fluya.
      // Esto permite que el SpeechRecognition también reciba el audio.
...
function provideFeedback(duration, volume) {
    ...
    if (wordsPerMinute > 0 && wordsPerMinute < 60) {
        feedback.rhythm = "¡Lee un poquito más rápido! ";
    } else if (wordsPerMinute > 150) {
        feedback.rhythm = "¡Intenta leer un poco más despacio para que se entienda mejor! ";
    } else {
        feedback.rhythm = "Tu ritmo de lectura es bueno. ";
    }

    if (volume < 30) {
        feedback.volume = "Necesitas hablar un poco más fuerte. ¡Usa tu voz de león! ";
    } else if (volume > 90) {
        feedback.volume = "Estás hablando muy fuerte, ¡intenta un tono más suave! ";
    } else {
        feedback.volume = "Tu volumen es adecuado. ";
    }
    
    const linesRead = currentLine;
    if (feedback.rhythm.includes("bueno") && feedback.volume.includes("adecuado")) {
        feedback.overall = "¡Lo hiciste muy, muy bien! ¡Felicidades! 🥳";
    } else if (linesRead < poemLines.length / 2) {
         feedback.overall = "Sigue practicando. ¡Cada vez lo harás mejor! 💪";
    } else {
        feedback.overall = "¡Buen intento! Sigue practicando para mejorar. 👍";
    }

    let message = `${feedback.overall}<br><br>${feedback.rhythm}${feedback.volume}`;
...
function startKaraoke() {
  ...
  if (!SpeechRecognition) {
    alert("Tu navegador no soporta reconocimiento de voz. Usa Chrome o Edge para esta función.");
    return;
  }
  ...
  recognition.onend = () => {
    if (mediaRecorder && mediaRecorder.state === "recording" && currentLine < poemLines.length) {
      recognition.start();
    }
  };
